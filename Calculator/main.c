...
// flags to avoid duplication of dispaying a digit multiple times
...
void main(void){
	...
	for(;;){
		...
		// move displayed digits to the left
		MoveDigits_4();
		...
		// choose which digit will be dispalayed in this scope
		SwitchDisplayedDigit(cnt_4);

		// check whisch button is clicked right now
		FindPressedButton();

		// set flags to prevent duplication of dispaying a digit multiple times
